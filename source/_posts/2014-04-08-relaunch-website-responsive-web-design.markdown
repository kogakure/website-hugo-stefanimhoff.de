...
Selbstverständlich ist die komplette Website jetzt auch in *Responsive Web Design* gebaut, etwas anderes sollte man 2014 nicht mehr abliefern.

## Gestaltung der Website

### Typographie
Beim Design habe ich dieses Mal besonders viel Wert auf die Typographie gelegt. Während der Design-Phase hatte ich monatelang hatte ich nichts weiter fertiggestellt als die Typographie und den Inhalt. Diesen Ansatz, bei dem von innen nach aussen gestaltet wird, wollte ich unbedingt ausprobieren.
...
Außerdem wollte ich von Anfang an gleich mit einer Skala arbeiten. Dies ist eine uralte Technik, die seit Jahrhunderten im Schriftsatz eingesetzt wird. Dabei wird ein besonderes Harmonieverhältnis ausgewählt und daran dann alle Breiten und Größen festgelegt. Ich verwende den *Goldenen Schnitt* (Verhältnis von 1:1,618) als Grundlage für meine Skala.
...
Die Grids sind komplett fluid und mehrere Breakpoints (durch Media Queries gesteuert) sorgen dafür, dass Lesebreite und Layout der Seite auf jeder Auflösung – vom Smartphone bis zum Desktop – immer optimal bleiben.
...
Für alle Piktogramme und Icons setze ich komplett auf Vektorfonts. Dabei wird die SVG-Grafik in eine Schriftart konvertiert. Vorteil dieser Technik ist, dass für eine große Anzahl an Icons nur ein Server-Request benötigt wird.
...
Aber auch ein komplett neues Favicon, Apple Touch Icons und Windows&nbsp;8-Tiles gibt es jetzt.
...
### Asset Pipeline
Für eine optimale Performance wollte ich von Anfang an, dass alle meine Assets (Bilder, Stylesheets, JavaScript, …) gecached, minimiert und optimiert werden. Für Rails ist die [Asset-Pipeline](http://guides.rubyonrails.org/asset_pipeline.html) seit einiger Zeit der Standard. Diese kombiniert mehrere Dateien zusammen, minimiert und komprimiert JavaScript und Stylesheets und versieht Bilder mit so genannten Cache-Keys, damit diese auch wieder invalidiert werden können. Gottseidank gibt es mehrere Projekte, die dieses für Jekyll portieren. Ich setze hierfür [Jekyll-Assets](https://github.com/ixti/jekyll-assets/) ein.

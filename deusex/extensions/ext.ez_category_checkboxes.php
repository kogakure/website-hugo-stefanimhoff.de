...
class Ez_category_checkboxes {
	...
	function fetch_group_name($row)
	{
		...
		if ($IN->GBL('C') == 'publish' && ($IN->GBL('M') == '' ||  $IN->GBL('M') == 'entry_form' || $IN->GBL('M') == 'new_entry') || $IN->GBL('C') == 'edit' && ($IN->GBL('M') == 'edit_entry' || $IN->GBL('M') == 'new_entry'))
		{
			...
			if ($cat_group != '')
			{
				if (! isset($SESS->cache['ez_category_checkboxes']['group_names']))
				{
					$group_ids = str_replace('|', ',', $cat_group);
					$group_names = NULL;
					
					$query = $DB->query("SELECT group_name FROM exp_category_groups WHERE group_id IN (".$group_ids.")");
					...
					$SESS->cache['ez_category_checkboxes']['group_names'] = $group_names;
				}
			}
	...
	function create_category_table($out='')
	{
		...
		if ($IN->GBL('C') == 'publish' && ($IN->GBL('M') == '' ||  $IN->GBL('M') == 'entry_form' || $IN->GBL('M') == 'new_entry') || $IN->GBL('C') == 'edit' && ($IN->GBL('M') == 'edit_entry' || $IN->GBL('M') == 'new_entry'))
		{
			...
			if (isset($SESS->cache['ez_category_checkboxes']['group_names']))
			{
				$group_names = $SESS->cache['ez_category_checkboxes']['group_names'];
